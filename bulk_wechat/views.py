...
class ExportRecipientToCSVView(LoginRequiredMixin, PermissionRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        recipients = WeChatRecipient.objects.filter(id__in=selected_ids)

        response = HttpResponse(content_type='text/csv')
        response['Content-Disposition'] = 'attachment; filename="wechat_recipients.csv"'

        writer = csv.writer(response)
        writer.writerow(['ID', 'Recipient Name', 'Recipient ID', 'Category'])

        for r in recipients:
            writer.writerow([r.pk, r.name, r.recipient_id, r.category.name if r.category else ''])
